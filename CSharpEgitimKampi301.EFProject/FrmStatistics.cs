...
namespace CSharpEgitimKampi301.EFProject
{
    public partial class FrmStatistics : Form
    {
        ...
        private void FrmStatistics_Load(object sender, EventArgs e)
        {
            ...
            lblRomeGuideName.Text = db.Guide.Where(x => x.GuideId == romeGuideId).Select(y => y.GuideName + " " + y.GuiderSurname).FirstOrDefault().ToString();
            ...
            var guideIdByNameAysegulKoca = db.Guide.Where(x => x.GuideName == "Ayşegül" && x.GuiderSurname == "Koca").Select(y => y.GuideId).FirstOrDefault();
            lblAysegülKocaLocationCount.Text = db.Location.Where(x => x.GuideId == guideIdByNameAysegulKoca).Count().ToString();
